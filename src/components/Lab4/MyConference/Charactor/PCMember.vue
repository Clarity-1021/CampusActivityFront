      ...
      <div class="DisplayBox mt-12 mt-md-0">
        <v-tabs v-model="tab" color="#1abc9c" background-color="transparent" show-arrows>
          ...
          <v-tabs-items v-model="tab">
            <v-tab-item>
              <CheckPaper v-if="stage==='viewing'||stage==='view_end'" :conference_id="conference_id" :type="1"/>
              <p v-else>初审处于关闭状态</p>
            </v-tab-item>
            <v-tab-item>
              <CheckPaper v-if="stage==='reviewing'||stage==='review_end'" :conference_id="conference_id" :type="2"/>
              <p v-else>复审处于关闭状态</p>
            </v-tab-item>
            <v-tab-item>
              <MyPaper />
    ...
    methods:{
      ...
      getInfo(){
        ...
        this.$axios.get('/conference_detail?conference_id='+this.conference_id)
          .then(resp => {
            this.stage=resp.data.conference_stage;
          })
          .catch(error => {
            console.log(error);
          });
