          ...
          <v-container class="displayConfDetail">
                  ...
                  <v-expansion-panel-header><!--简要条目陈列-->
                    <v-row align="center" class="spacer" no-gutters>
                      ...
                      <v-col class="text-center d-flex justify-space-around" cols="3">
                        <el-button v-if="conference.conference_stage!=='end'&&conference.status==='unprocessed'" type="success" style="margin: 0; color: white" class="recBtn-medium" size="small" @click="process(conference.conference_id)">同意</el-button>
                        <el-button v-if="conference.conference_stage!=='end'&&conference.status==='unprocessed'" type="danger" style="margin: 0; color: white" class="recBtn-medium" size="small" @click="refuse(conference.conference_id)">拒绝</el-button>

                        <el-tag v-if="conference.status==='accept'" class="tag-default" type="info">已同意</el-tag>
                        <el-tag v-if="conference.status==='refuse'" class="tag-default" type="info">已拒绝</el-tag>
                        <el-tag v-if="conference.conference_stage==='end' && conference.status!=='refuse' && conference.status!=='accept'" class="tag-default" type="info">无法处理</el-tag>
                      </v-col>
                    </v-row>
                  </v-expansion-panel-header>
            ...
            <el-dialog title="选择话题" :visible.sync="acceptDialogFlag">
              ...
              <el-row class="mt-4 d-flex justify-end">
                <el-button type="primary" style="color: white" size="small" disabled v-if="topicChosen.length === 0">确认</el-button>
                <el-button type="primary" size="small" style="color: white" @click="chooseTopic" v-else>确认</el-button>
